...
#todo check id jack is hessian or hessian*grad
res_4_prev = np.linalg.norm((gradient(X, labels, w_i) - gradient(X, labels, w_0) - eps * mul(
    d.transpose(), (hessian(X, w_0)))))  # todo check
print("A")
for i in range(1, limit):
    ...
    #todo first part is vector gradient matrix. todo norm not abs?
    ...
    #res_3 = abs((f(X,labels,w_i) - f(X,labels,w_0) - jackMV))
    res3 = np.linalg.norm(gradient(X, labels, w_i) - gradient(X, labels, w_0))  #todo first part is marix 784*1000, hessian matrix 784*784. todo norm not abs?
    res4 = np.linalg.norm((gradient(X, labels, w_i) - gradient(X, labels, w_0) - eps_i * mul(d.transpose(), (hessian(X, w_0)))))  # todo check
